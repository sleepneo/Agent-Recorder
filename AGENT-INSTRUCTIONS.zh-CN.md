...
本版还包含默认关闭的系统声音受控预览。只有 Agent Recorder 进程启动前已设置 `AGENT_RECORDER_EXPERIMENTAL_SYSTEM_AUDIO=true` 时，`audio.system_audio.enabled=true` 才能进入正常的选区、确认、倒计时和录制流程；否则仍在任何 UI 或捕获前返回 `CAPABILITY_NOT_IMPLEMENTED`。未指定 `audio.system_audio.device_id` 时使用当前 Windows 多媒体默认 render endpoint。麦克风和系统声音不能在同一请求中同时启用。该预览尚未翻转公开能力契约，因此 agent 不应依据默认 `/capabilities` 响应自动假定其可用。
